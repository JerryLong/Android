...
public class BitmapUtils {
	...
	public static Bitmap blur(Context context, Bitmap sentBitmap, int radius) {
		...
		if (radius < 0) {
			radius = 0;
			if (DEBUG) {
				L.d(TAG, "radius must be 0 < r < 25 , forcing radius=0");
			}
		} else if (radius > 25) {
			radius = 25;
			if (DEBUG) {
				L.d(TAG, "radius must be 0 < r < 25 , forcing radius=25");
			}
		}
	 ...
	 * @return
	 */
	public static byte[] yuvLandscapeToPortrait(byte[] sourceData, int width,
			int height) {
